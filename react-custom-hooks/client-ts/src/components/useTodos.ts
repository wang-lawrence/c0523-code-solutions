...
export function useTodos(): UseTodosValues {
  ...
  async function toggleCompleted(todoId: number): Promise<void> {
     ...
     */
    try {
      const editItem = todos?.find((todo) => todo.todoId === todoId);
      if (!editItem) {
        throw new Error('Task item not found');
      }
      editItem.isCompleted = !editItem.isCompleted;
      const updatedTodo = await updateTodo(editItem);
      const updatedTodos = todos?.map((originalTodo) =>
        todoId === originalTodo.todoId ? updatedTodo : originalTodo
      );
